...
export class MerchandiseService {
  ...
  public get_item(id: number): Observable<Merchandise> {
    return this.httpService.get<Merchandise>(`http://api.saltandginger.betazeta.io/api/merchandise/${id}`).pipe(
      map(data => new Merchandise().deserialize(data))
    );
  }

  public get_all_items(): Observable<Merchandise[]> {
    return this.httpService.get<Merchandise[]>(`http://api.saltandginger.betazeta.io/api/merchandise`).pipe(
